...
class Lista {
    ...
    public void bolha(Lista series, int n) {
        int i, j;

        for(i = (n - 1); i > 0; i--) {
            for(j = 0; j < i; j++) {
                if(series.array[j].getNumeroTemporadas() > series.array[j + 1].getNumeroTemporadas() || series.array[j].getNumeroTemporadas() == series.array[j + 1].getNumeroTemporadas() && series.array[j].getNome().compareTo(series.array[j + 1].getNome()) > 0) {
                    comparacoes += 3;
                    swap(j, j + 1, series);
                }
            }
